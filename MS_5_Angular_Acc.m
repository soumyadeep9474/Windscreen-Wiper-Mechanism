...
for i=1:length(t)
    ...
    if(i < length(t))
        ...
        delete(time);
        vel = subplot(2,1,2);
        plot(vel,t(1:i),ang_acc_D(1:i));
        set(vel,'XLim',[0 7],'YLim',[-40 40]);
        xlabel(vel,'Time (s)');
        ylabel(vel,'Angular Accelaration (rad/s)');
        title(vel,'Angular Accelaration vs Time (input Angular Acc. = 0 )');
        grid on;
